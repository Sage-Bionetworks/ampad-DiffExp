...
enrich.meta = fread(synGet('syn11973632')@filePath, data.table = F) %>%
...
tmp = dplyr::filter(enrich, FDR <= 0.05, Sex == 'ALL', Direction == 'UP') %>% dplyr::group_by(SetName) %>% dplyr::summarise(count = n()) %>% dplyr::arrange(desc(count))


%>%
  dplyr::group_by(Model, Tissue, Sex, Direction) %>%
  dplyr::arrange(desc(Odds.Ratio))

p = ggplot(enrich.all, aes(y = SetName, x = Tissue, color = -log10(FDR), size = Odds.Ratio)) + geom_point()
p = p + facet_grid(Direction+.~Sex) + xlab('') 
p = p + theme_bw() %+replace% theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), 
                                    legend.position = 'top')
p = p + scale_color_gradientn(colours = c('grey', 'grey', viridis::viridis(2)[2], 
