...
library(synapser) # new synapse R client from install.packages("synapser", repos=c("https://sage-bionetworks.github.io/staging-ran", 'http://cran.fhcrc.org'))
...
# Github link
thisFileName <- 'ROSMAP_geneLevel.Rmd'
...
# Create restricted acls
synSetEntityACLs <- function(entity){
  superUser = c('UPDATE', 'CHANGE_SETTINGS', 'DELETE', 'READ', 'DOWNLOAD', 'MODERATE', 'CREATE', 'CHANGE_PERMISSIONS')
  synSetPermissions(entity, principalId = '3319864', accessType = superUser, overwrite = T)
  lapply(c('274008', '273995', '3327871', '3346847'), function(pids){
    synSetPermissions(entity, principalId = pids, accessType = superUser)
  })
  synSetPermissions(entity, principalId = '3344250', accessType = c('READ', 'DOWNLOAD'))
}
...
# Set annotations
annotList = synGetAnnotations(COUNT_ID)
annotList = lapply(annotList,function(x){unlist(x)})
annotList$normalizationStatus	= TRUE
annotList$normalizationType	= 'CQN'
...
annotList$fileFormat = 'tsv'
annotList$dataSubType = 'covariates'
tmp = synSetAnnotations(COV_OBJ, annotations = annotList)
tmp = synSetEntityACLs(COV_OBJ$properties$id)
...
COUNT_OBJ = synStore(COUNT_OBJ, activity = synGetProvenance(COV_OBJ$properties$id))
annotList$dataSubType = 'filteredCounts'
tmp = synSetAnnotations(COUNT_OBJ, annotations = annotList)
...
ECOUNT_OBJ = synStore(ECOUNT_OBJ, activity = synGetProvenance(COV_OBJ$properties$id))
annotList$dataSubType = 'estimatedCounts'
tmp = synSetAnnotations(ECOUNT_OBJ, annotations = annotList)
...
LCOUNT_OBJ = synStore(LCOUNT_OBJ, activity = synGetProvenance(COV_OBJ$properties$id))
annotList$dataSubType = 'filteredLogCPM'
tmp = synSetAnnotations(LCOUNT_OBJ, annotations = annotList)
...
OFFSET_OBJ = synStore(OFFSET_OBJ, activity = synGetProvenance(COV_OBJ$properties$id))
annotList$dataSubType = 'offset'
tmp = synSetAnnotations(OFFSET_OBJ, annotations = annotList)
...
DM_OBJ = synStore(DM_OBJ, activity = synGetProvenance(COV_OBJ$properties$id))
annotList$dataSubType = 'designMatrix'
tmp = synSetAnnotations(DM_OBJ, annotations = annotList)
tmp = synSetEntityACLs(DM_OBJ$properties$id)
...
SURR_OBJ = synStore(SURR_OBJ, activity = synGetProvenance(COV_OBJ$properties$id))
annotList$dataSubType = 'surrogateVariables'
tmp = synSetAnnotations(SURR_OBJ, annotations = annotList)
...
eEXP_OBJ = synStore(eEXP_OBJ, activity = synGetProvenance(COV_OBJ$properties$id))
annotList$dataSubType = 'residualGeneExpForeQTL'
tmp = synSetAnnotations(eEXP_OBJ, annotations = annotList)
...
nEXP_OBJ = File('ROSMAP_DLPFC_netResidualExpression.tsv', 
                ...
                parentId = CODE$properties$id)
nEXP_OBJ = synStore(nEXP_OBJ, activity = synGetProvenance(COV_OBJ$properties$id))
annotList$dataSubType = 'residualGeneExpForNetAnlz'
tmp = synSetAnnotations(nEXP_OBJ, annotations = annotList)
...
DEXP_OBJ = File('ROSMAP_DLPFC_DiffExpression.tsv', name = 'Differential Expression Results (cogdx)', 
                parentId = CODE$properties$id)
DEXP_OBJ = synStore(DEXP_OBJ, activity = synGetProvenance(COV_OBJ$properties$id))
annotList$dataSubType = 'differentialExpression'
tmp = synSetAnnotations(DEXP_OBJ, annotations = annotList)
...
FIT_OBJ = File('ROSMAP_Models.RData', 
               ...
               parentId = CODE$properties$id)
FIT_OBJ = synStore(FIT_OBJ, activity = synGetProvenance(COV_OBJ$properties$id))
annotList$dataSubType = 'differentialExpression'
annotList$fileFormat = 'RData'
tmp = synSetAnnotations(FIT_OBJ, annotations = annotList)
tmp = synSetEntityACLs(FIT_OBJ$properties$id)
```
|  *Results*                                  |  *SynapseID*                     |
|  -----------------------------------------  |   -----------------------------  |
...
obj = rmarkdown::render('ROSMAP_geneLevel.Rmd', output_format = 'html_document')
obj = File(obj, name = "Analysis results markdown", parentId = "syn8456629")
obj = synStore(obj, executed = thisFile)
# wikiPg = WikiPage(owner = obj, title = 'Figure1')
wikiPg = synGetWiki(CODE$properties$id)
