...
COUNT <- synapseClient::synGet(COUNT_ID)@filePath %>%
...
METADATA_TECH <- synapseClient::synGet(METADATA_TECH_ID)@filePath %>%
...
METADATA_AGE <- synapseClient::synGet(METADATA_AGE_ID)@filePath %>%
  read.table(sep='\t', header=T)

# Get clinical metadata
METADATA_CLINICAL_ID <- 'syn6101474'
ALL_USED_IDs[length(ALL_USED_IDs)+1] = METADATA_CLINICAL_ID
METADATA_CLINICAL <- synapseClient::synGet(METADATA_CLINICAL_ID)@filePath %>%
  read.csv(header = T)
...
synChangeEntityACL <- function(id, new.racl){
  tryCatch({ 
    ...
    old.acl = synapseClient::synGetEntityACL(id)
...
# Set annotations
all.annotations = annotations(synapseClient::synGet(COUNT_ID))
